    ...
    for itr in range(n_itrs):
        ...
        if not natural:
            ...
            def compute_fisher_matrix(theta, get_grad_logp_action, all_observations, all_actions):
                ...
                for ob,a in zip(all_observations, all_actions):
                    grad_log_p_action = get_grad_logp_action(theta, ob, a).flatten()
                    F += np.outer(grad_log_p_action, grad_log_p_action)
                return F/len(all_observations)
            ...
            def compute_natural_gradient(F, grad, reg=1e-4):
                ...
                natural_grad_flat = np.linalg.inv(F + np.eye(F.shape[0])*reg).dot(grad.flatten())
                return natural_grad_flat.reshape(grad.shape)
            ...
            def compute_step_size(F, natural_grad, natural_step_size):
                ...
                return np.sqrt(2*natural_step_size/(natural_grad_flat.dot(F.dot(natural_grad_flat))))
